...
export default function ProgramChairPastReportsScreen({ navigation }) {
    ...
    async function load() {
      try {
        ...
        const generated = courseRows.map((course, index) => {
          const type = index % 3 === 0 ? "Program Detailed" : "Program Summary";
          const year = index % 2 === 0 ? "2025" : "2024";
          return {
            id: `${course.code}-${index}`,
            title:
              type === "Program Detailed"
  ...
  return (
    <AppScreen
      title={"Past\nReports"}
      subtitle="View and download previously submitted program assessment reports."
      heroFooter={
