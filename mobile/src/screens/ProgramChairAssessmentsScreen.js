...
export default function ProgramChairAssessmentsScreen({ navigation }) {
  ...
  const filteredSections = useMemo(() => {
    return sections.filter((section) => {
      ...
      return matchesSO && matchesCourse && matchesSection && matchesSemester && matchesFaculty && matchesYear;
    });
  }, [
    sections,
    courseMappings,
  ...
  const coursesForGrid = useMemo(() => {
    ...
    filteredSections.forEach((section) => {
      ...
      course.studentCount += section.students.length;
    });
              ...
              coursesForGrid.map((course) => {
                ...
                return (
                  <Pressable
                    key={course.id}
                    onPress={() =>
                      navigation.navigate("ProgramChairAssessmentEntry", {
                        course,
                        studentOutcomes,
                        courseMappings,
                        summaryMap,
  ...
  useEffect(() => {
    ...
    async function loadGradesForSelection() {
      ...
      try {
        setLoading(true);
        setError("");
        const baseStudents = (selectedSection.students || []).map((student) => ({
          ...student,
          grades: {},
        }));
